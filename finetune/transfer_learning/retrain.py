# NOTE(gnashcraft):
# Modifications have been made to the below code in order to work with GCP Machine Learning
...
#ngashcraft
import time
WAIT = 5 # wait 5 seconds for gcs access
...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
  ...
  while True:
    try:
      ...
      print('Caught GCS UnavailableError for {}'.format(image_dir))
      time.sleep(WAIT)
  result = {}
  # ngashcraft
  # sub_dirs = [x[0] for x in gfile.Walk(image_dir)]
  sub_dirs = FLAGS.classes
...
def ensure_dir_exists(dir_name):
  ...
  while True:
    try:
      ...
      print('Caught GCS UnavailableError for {}'.format(dir_name))
      time.sleep(WAIT)
...
def create_bottleneck_file(bottleneck_path, image_lists, label_name, index,
  ...
  while True:
    try:
      ...
      print('Caught GCS UnavailableError for {}'.format(image_path))
      time.sleep(WAIT)
  ...
  with tf.gfile.FastGFile(bottleneck_path, 'w') as bottleneck_file:
      bottleneck_file.write(bottleneck_string)
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, image_dir,
  ...
  while True:
    try:
      if not tf.gfile.Exists(bottleneck_path):
      ...
      print('Caught GCS UnavailableError for {}, waiting...'.format(bottleneck_path))
      time.sleep(WAIT)
  #ngashcraft
  # with open(bottleneck_path, 'r') as bottleneck_file:
  #   bottleneck_string = bottleneck_file.read()
  with tf.gfile.FastGFile(bottleneck_path, 'r') as bottleneck_file:
      bottleneck_string = bottleneck_file.read()
  did_hit_error = False
  try:
...
def main(_):
  ...
  while True:
    try:
      ...
      print('Caught GCS UnavailableError for {}'.format(FLAGS.summaries_dir))
      time.sleep(WAIT)

  # Set up the pre-trained graph.
  #ngashcraft
  # maybe_download_and_extract()
  print('Creating Inception Graph') #ngashcraft
  graph, bottleneck_tensor, jpeg_data_tensor, resized_image_tensor = (
      create_inception_graph())

  # Look at the folder structure, and create lists of all the images.
  print('Creating Image Lists') #ngashcraft
  ...
  if do_distort_images:
    ...
    # cached them on disk.
    print('Caching Bottlenecks') #ngashcraft
    #ngashcraft
    cache_bottlenecks(sess, image_lists, FLAGS.image_dir, FLAGS.bottleneck_dir,
                      jpeg_data_tensor, bottleneck_tensor)

  # Add the new layer that we'll be training.
  print('Adding Final Training Ops') #ngashcraft
  ...
  print('Adding Evaluation Ops') #ngashcraft
  ...
  print('Beginning Training Loop') #ngashcraft
  for i in range(FLAGS.how_many_training_steps):
    ...
    if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
      ...
      validation_writer.flush() #ngashcraft
  ...
  print('Saving output graph and labels') #ngashcraft
